...
export default function Pemasukan() {
  ...
  useEffect(() => {
    fetch("http://127.0.0.1:8000/api/transaksi")
      .then((res) => res.json())
      .then((data) => {
        const filtered = data.filter((item) => item.jenis === "pemasukan");
        setPemasukan(filtered);
      })
      .catch((err) => console.error(err));
  }, []);

  const filteredData = pemasukan.filter(
    (item) =>
  ...
  return (
    <div className="min-h-screen flex bg-white font-sans">
      ...
      >
        <div className="pb-6 border-b border-white/10 mb-5">
          <h1 className="text-white text-[18px] font-bold tracking-wide">
            E-Wallet
          </h1>
          <div className="mt-1 h-0.5 w-10 bg-white/40 rounded-full"></div>
        </div>
        ...
        <nav className="flex-1 space-y-3">
          {navItems.map((item) => (
            <button
              ...
              className={`w-full text-left flex items-center justify-between px-4 py-3 rounded-xl text-[13px] font-medium transition-all duration-150
              ${
                item.key === "pemasukan"
